...
void loop() {
    ...
    if ((mpuIntStatus & 0x10) || fifoCount == 1024) {
        ...
        // *** The core change is here. We get the quaternion and send it directly. ***
        mpu.dmpGetQuaternion(&q, fifoBuffer);
        
        // Get Temperature
        int16_t tempRaw = mpu.getTemperature();
        float tempF = ((tempRaw / 340.0) + 36.53) * 9.0/5.0 + 32.0;

        // Get Real Acceleration (without gravity)
        mpu.dmpGetGravity(&gravity, &q);
        mpu.dmpGetAccel(&aa, fifoBuffer);
        mpu.dmpGetLinearAccel(&aaReal, &aa, &gravity);
        float accelMagnitude = sqrt(pow(aaReal.x / 16384.0, 2) + pow(aaReal.y / 16384.0, 2) + pow(aaReal.z / 16384.0, 2));

        // --- UPDATED: Send all 6 data points (w,x,y,z,temp,accel) ---
